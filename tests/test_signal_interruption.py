...
class TestSignalInterruption(unittest.TestCase):
    ...
    def test_no_signal_fn_sleep_called_per_scenario(
        ...
        """When get_signal_fn is None, time.sleep() is called for each tick
        with a value in (0, 1] — the remaining slice of the deadline."""
        self._setup_signal_ctx(mock_signal_ctx)

        # 2 scenarios × 3 ticks each
        # Each pair (loop-condition check, sleep-arg computation) returns
        # values that keep the loop running for 3 ticks then exits.
        mock_monotonic.side_effect = [
            0.0, 0.0, 1.0, 2.0, 3.0,
            0.0, 0.0, 1.0, 2.0, 3.0,
        ]
        ...
        # sleep() must have been called at least once per scenario
        self.assertGreater(mock_sleep.call_count, 0)
        # Every sleep argument must be in the valid range (0, 1]
        for call_args in mock_sleep.call_args_list:
            sleep_val = call_args[0][0]
            self.assertGreater(sleep_val, 0, "sleep value must be positive")
            self.assertLessEqual(sleep_val, 1, "sleep value must not exceed 1 second")
