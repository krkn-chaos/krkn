...
from kraken.plugins.network import cerberus
import typing
...
class NetworkScenarioConfig:
    ...
    node_interface_name: typing.Dict[str, typing.List[str]
     ] = field(
        default=None,
        metadata={
            "name": "Node Interface Name",
            "description": "Dictionary with node names as key and values as a list of their test interfaces. Required if label_selector is not set.",
        }
    )
...
def verify_interface(input_interface_list: typing.List[str], node: str, pod_template, cli: CoreV1Api) -> typing.List[str]:
    """
    Function that verifies whether a list of interfaces is present in the node. If the
    list is empty, it fetches the interface of the default route
        ...
        if input_interface_list == []:
            ...
            for interface in input_interface_list:
                if interface not in node_interface_list:
                    logging.error("Interface %s not found in node %s interface list %s" % (interface, node, node_interface_list))
                    raise Exception("Interface %s not found in node %s interface list %s" % (interface, node, node_interface_list))
    finally:
...
def get_node_interfaces(node_interface_dict: typing.Dict[str, typing.List[str]], label_selector: str, instance_count: int, pod_template, cli: CoreV1Api) -> typing.Dict[str, typing.List[str]]:    

    """
    Function that is used to process the input dictionary with the nodes and its test
    interfaces. 
    ...
    If the dictionary is not empty, it is filtered to include the nodes which are 
    active and then their interfaces are verified to be present
 

    Args:
        node_interface_dict (Dictionary with keys as node name and value as a list of interface names)
    ...
    if not node_interface_dict:
        ...
        for node in node_name_list:
           filtered_node_list.extend(kube_helper.get_node(node, label_selector, instance_count, cli))
...
def apply_ingress_filter(cfg: NetworkScenarioConfig, interface_list: typing.List[str], node: str, pod_template, job_template, batch_cli: BatchV1Api, cli: CoreV1Api, create_interfaces: bool = True, param_selector: str ='all') -> str:

    """
    Function that applies the filters to shape incoming traffic to the provided node's interfaces.
    This is done by adding a virtual interface before each physical interface and then performing egress
    traffic control on the virtual interface 
    ...
    Returns:
        The name of the job created that executes the commands on a node for ingress chaos scenario
    """
...
def create_virtual_interfaces(cli: CoreV1Api, interface_list: typing.List[str], node: str, pod_template) -> None:
    """
    Function that creates a privileged pod and uses it to create virtual interfaces on the node
...
def delete_virtual_interfaces(cli: CoreV1Api, node_list: typing.List[str], pod_template):
    """
    Function that creates a privileged pod and uses it to delete all virtual interfaces on the 
    specified nodes
...
def wait_for_job(batch_cli: BatchV1Api, job_list: typing.List[str], timeout: int = 300) -> None:
    """
    ...
    while count != job_len:
        for job_name in job_list:
            try:
                api_response = kube_helper.get_job_status(batch_cli, job_name, namespace="default")
            ...
            if time.time() > wait_time:
                raise Exception("Jobs did not complete within the {0}s timeout period".format(timeout))
            time.sleep(5)
...
def delete_jobs(cli: CoreV1Api, batch_cli: BatchV1Api, job_list: typing.List[str]):
    ...
    for job_name in job_list:
        try:
            ...
            logging.warn("Exception in getting job status")
        api_response = kube_helper.delete_job(batch_cli, name=job_name, namespace="default")


def get_ingress_cmd(interface_list: typing.List[str], network_parameters: typing.Dict[str, str], duration: int = 300):
    """
    Function that returns the commands to the ingress traffic shaping on the node.
    First, the virtual interfaces created are linked to the test interfaces such that
    there is a one-to-one mapping between a virtual interface and a test interface.
    Then, incoming  traffic to each test interface is forced to first pass through the
    corresponding virtual interface. Linux's tc commands are then used to performing
    egress traffic control on the virtual interface. Since the outbound traffic from
    the virtual interface passes through the test interface, this is effectively ingress
    traffic control. After a certain time interval, the traffic is restored to normal
...
def network_chaos(cfg: NetworkScenarioConfig) -> typing.Tuple[str, typing.Union[NetworkScenarioSuccessOutput, NetworkScenarioErrorOutput]]:
    ...
    try:
        node_interface_dict = get_node_interfaces(cfg.node_interface_name, cfg.label_selector, cfg.instance_count, pod_interface_template, cli)
    except:
    ...
    try:
        if cfg.execution_type == 'parallel':
            for node in node_interface_dict:
                job_list.append(apply_ingress_filter(cfg, node_interface_dict[node], node, pod_module_template, job_template, batch_cli, cli))
            ...
            for param in cfg.network_params:
                for node in node_interface_dict:
                    job_list.append(apply_ingress_filter(cfg, node_interface_dict[node], node, pod_module_template,job_template, batch_cli, cli, create_interfaces=create_interfaces, param_selector=param))
