...
# case, infact kubeconfig contains filesystem paths to those files, this functions provide to build an arcaflow-compatible
...
def buildArcaKubeConfig(kubeconfig_path: str) -> str:
    ...
    if kubeconfig["users"][userId]["user"]["client-certificate"] is not None:
    ...
    if kubeconfig["users"][userId]["user"]["client-key"] is not None:
    ...
    if kubeconfig["clusters"][clusterId]["cluster"]["certificate-authority"] is not None:
