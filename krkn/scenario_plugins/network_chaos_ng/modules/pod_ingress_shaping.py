...
from krkn.scenario_plugins.network_chaos_ng.modules.utils_network_shaping import get_bridge_name, get_test_pods, \
    check_bridge_interface, apply_net_policy, wait_for_job, delete_jobs, apply_ingress_policy, delete_virtual_interfaces
...
class PodIngressShapingModule(AbstractNetworkChaosModule):
    ...
    def run(self, target: str, error_queue: queue.Queue = None):
        ...
        test_pod_name = self.config.pod_name
        job_list = []
        ip_set = set()
        node_dict = {}
        label_set = set()
        ...
        try:
            ...
            api_ext = client.ApiextensionsV1Api(self.kubecli.get_lib_kubernetes().api_client)
            custom_obj = client.CustomObjectsApi(self.kubecli.get_lib_kubernetes().api_client)
            br_name = get_bridge_name(api_ext, custom_obj)
            pods_list = get_test_pods(
                test_pod_name, test_label_selector, test_namespace, self.kubecli.get_lib_kubernetes()
            )
            ...
            for pod_name in pods_list:
                pod_stat = self.kubecli.get_lib_kubernetes().read_pod(pod_name, test_namespace)
            ...
            check_bridge_interface(
                list(node_dict.keys())[0], pod_module_template, br_name, self.kubecli.get_lib_kubernetes()
            )
            ...
            for mod in mod_lst:
                for node, ips in node_dict.items():
                    job_list.extend(
                        apply_ingress_policy(
                            ...
                            job_template,
                            pod_module_template,
                            {"latency": self.config.latency, "bandwidth":self.config.bandwidth, "loss":self.config.loss},
                ...
                if self.config.network_shaping_execution == "serial":
                    ...
                    wait_for_job(job_list[:], self.kubecli.get_lib_kubernetes(), self.config.test_duration + 300)
            ...
            delete_virtual_interfaces(self.kubecli.get_lib_kubernetes(), node_keys, pod_module_template)
