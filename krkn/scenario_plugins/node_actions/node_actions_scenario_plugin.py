...
#
# NOTE: stop_kubelet_scenario uses node_reboot_scenario for rollback because:
# - restart_kubelet_scenario uses 'oc debug node/' which requires kubelet running
# - When kubelet is stopped, the debug pod cannot start (chicken-and-egg problem)
# - node_reboot_scenario uses cloud provider APIs (out-of-band management)
# - This matches the existing stop_start_kubelet_scenario pattern
REVERSIBLE_ACTIONS = {
    ...
    def rollback_node_action(
        ...
        try:
            ...
            if reverse_action == "node_start_scenario":
                ...
                # Used for stop_kubelet rollback: reboot restarts kubelet via systemd
                soft_reboot = False  # Use hard reboot for reliability
                scenario_obj.node_reboot_scenario(1, node, timeout, soft_reboot)
            elif reverse_action == "restart_kubelet_scenario":
                # Legacy fallback (not used by default due to chicken-and-egg problem)
