...
class Serializer:
    ...
    def _parse_rollback_callable_code(self, callable: Callable) -> tuple[str, str]:
        """
    ...
    def _get_setup_krkn_lib(self, version: Version) -> str:
        ...
        if setup_lines:
            setup_lines.insert(
                0, "    kubeconfig_path = os.getenv('KUBECONFIG', '~/.kube/config')"
            )
            setup_lines.append("    \n")
    ...
    def _format_all_args(self, version: Version) -> str:
        ...
        if version.serialized_kwargs:
            args_parts.extend(
                [f"{k}={v}" for k, v in version.serialized_kwargs.items()]
            )
    ...
    def serialize_callable(self, callable: Callable, arguments=(), kwargs=None) -> str:
        ...
        version = Version(
            ...
            has_lib_telemetry=self._has_krkn_lib_obj(
                KrknTelemetryOpenshift, arguments, kwargs
            ),
            has_kubecli=self._has_krkn_lib_obj(KrknKubernetes, arguments, kwargs),
            has_scenario_telemetry=self._has_krkn_lib_obj(
                ScenarioTelemetry, arguments, kwargs
            ),
