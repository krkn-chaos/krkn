# Pod Kill Chaos Scenario

## Description
This scenario forcefully kills pods using different termination methods to test application resiliency and recovery mechanisms under various termination conditions.

## Risk Level
**High** - This will cause immediate pod termination without graceful shutdown.

## Target
Kubernetes Pods

## Usage

### Basic Usage
```bash
krkn run-template pod-kill
```

### With Custom Parameters
```bash
krkn run-template pod-kill \
  --param name_pattern="^nginx-.*$" \
  --param namespace_pattern="^production$" \
  --param kill=2 \
  --param kill_mode="signal" \
  --param signal="SIGTERM" \
  --param krkn_pod_recovery_time=180
```

## Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `name_pattern` | string | `^app-.*$` | Regex pattern to match pod names |
| `namespace_pattern` | string | `^default$` | Regex pattern to match namespace names |
| `kill` | integer | `1` | Number of pods to terminate |
| `kill_mode` | string | `"force"` | Kill mode: force, graceful, or signal |
| `signal` | string | `"SIGKILL"` | Signal to send (for signal mode) |
| `krkn_pod_recovery_time` | integer | `120` | Time to wait for pod recovery in seconds |

## Kill Modes

- **force**: Immediate pod deletion without graceful shutdown
- **graceful**: Allows pods to terminate gracefully (default Kubernetes behavior)
- **signal**: Sends specific signal to pod processes

## What Happens
1. KRKN identifies pods matching the name and namespace patterns
2. Applies the specified kill method to target pods
3. Monitors pod recovery for the specified duration
4. Reports on the success of pod recreation and application recovery

## Prerequisites
- Target pods must exist
- Sufficient permissions to terminate pods
- ReplicaSet or Deployment should be configured to recreate terminated pods
- Application should handle termination gracefully (if using graceful mode)

## Cleanup
No manual cleanup required - terminated pods should be automatically recreated by their controllers.

## Example Output
```
✓ Found 3 pods matching pattern ^app-.*$ in namespace default
✓ Killing pod app-pod-1 using force mode
✓ Waiting 120 seconds for pod recovery...
✓ Pod app-pod-1 successfully recreated
✓ All pods recovered successfully
```
