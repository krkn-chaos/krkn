"""
Functional tests for the I/O hog scenario (hog_scenarios), migrated from the legacy
CI/tests/test_io_hog.sh.

I/O hog targets nodes (not workloads): Krkn deploys a short-lived hog pod (name prefix
"io-hog-") onto each selected node, runs it for the configured duration, then deletes the
...
failure on invalid selector/config. They validate Krkn behavior and cleanup semantics, not
actual I/O throughput or disk pressure.
"""
...
class TestIoHog(BaseScenarioTest):
    ...
    @pytest.mark.order(1)
    def test_io_hog_success_lifecycle_and_targeting(self):
        ...
        if not nodes:
            pytest.skip("No schedulable worker node available for I/O hog targeting")
        node = nodes[0]
        ns = self.ns
        ...
        config_path = self.build_config(
            self.SCENARIO_TYPE, str(scenario_path), filename="io_hog_success_config.yaml"
        )
        ...
        try:
            ...
            assert pod is not None, (
                f"Expected an I/O hog pod (prefix {HOG_POD_PREFIX!r}) to be created in namespace={ns}"
            )
            assert pod.spec and pod.spec.node_name == node, (
                f"I/O hog pod {pod.metadata.name} scheduled on "
    ...
    @pytest.mark.no_workload
    @pytest.mark.order(2)
    def test_io_hog_invalid_selector_fails(self):
        ...
        scenario_path = self.write_scenario(self.tmp_path, scenario)
        config_path = self.build_config(
            self.SCENARIO_TYPE, str(scenario_path), filename="io_hog_invalid_selector_config.yaml"
        )
    ...
    @pytest.mark.no_workload
    @pytest.mark.order(3)
    def test_io_hog_invalid_config_fails(self):
        ...
        scenario_path = self.write_scenario(self.tmp_path, scenario)
        config_path = self.build_config(
            self.SCENARIO_TYPE, str(scenario_path), filename="io_hog_invalid_config_config.yaml"
        )
        result = self.run_kraken(config_path, timeout=KRAKEN_RUN_TIMEOUT)
        assert_kraken_failure(
