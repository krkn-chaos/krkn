...
- **10+ Ready-to-use Templates**: Pod failure, node failure, network latency, CPU stress, and more
- **Simple CLI Commands**: `krkn run-template pod-failure`
- **Parameter Customization**: Override defaults without editing files
- **Template Validation**: Ensure templates are correctly configured

### Quick Start with Chaos Library
...
krkn run-template pod-failure --param kill=2 --param namespace_pattern=^prod$
