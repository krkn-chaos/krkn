...
def main(cfg):
    ...
    try:
        ...
        logging.error("Cannot find a yaml file at %s, please check" % (cfg))
        sys.exit(1)
            ...
            for scenario in chaos_scenarios:
                ...
                if run_signal == "PAUSE":
                    while publish_running_status and run_signal == "PAUSE":
                        logging.info(
                            "Pausing Kraken run, waiting for %s seconds and will re-poll signal" % str(wait_duration)
                        )
